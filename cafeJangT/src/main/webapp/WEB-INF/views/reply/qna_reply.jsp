                ...
                <table class="table">                    
                    <tr>
                        <td>
                            ...
                            <br>
                            <div>
                                <a href='javascript:void(0)' onClick="return qna_save_comment('${qnaVO.qseq }')" class="btn pull-right btn-success">등록</a>
                            </div>
                        </td>
                    </tr>
                </table>
 ...
 * 상품평 등록
 */
function qna_save_comment(qseq) {
	if(document.commentForm.content.value=='') {
		...
		$.ajax({
			type: 'POST',
			url: 'qna_save_comment',
			data: $("#commentForm").serialize(),
			success: function(data) {
				if(data="success") {	// 댓글 등록 성공
					getQnaCommentList();	// 댓글 목록 요청 함수 호출
					$("#content").val("");
				}
			},		
			...
			if(data.length > 0) {
				for(i=0; i<data.length; i++) {
					if(data[i].a_id != null) {
						...
						html += "&nbsp;<span id=\"write_date\">" + qnaDisplayTime(data[i].regdate) + "</span><br>"
...
function qnaDisplayTime(timeValue) {
	var today = new Date();
