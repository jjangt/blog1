                ...
                <table class="table">                    
                    <tr>
                        <td>
                            ...
                            <br>
                            <div>
                                <a href='javascript:void(0)' onClick="return img_save_comment('${imgBoardVO.ibseq }')" class="btn pull-right btn-success">등록</a>
                            </div>
                        </td>
                    </tr>
                </table>
 ...
 * 상품평 등록
 */
function img_save_comment(ibseq) {
	if(document.commentForm.content.value=='') {
		...
		$.ajax({
			type: 'POST',
			url: 'img_save_comment',
			data: $("#commentForm").serialize(),
			success: function(data) {
				if(data="success") {	// 댓글 등록 성공
					getImgCommentList();	// 댓글 목록 요청 함수 호출
					$("#content").val("");
				}
			},		
 ...
 * 댓글 목록 불러오기
 */
function getImgCommentList() {
	$.ajax({
		type: 'GET',
		url: 'img_comment_list',
		dataType: "json",
		data: $("#commentForm").serialize(),
			...
			if(data.length > 0) {
				for(i=0; i<data.length; i++) {
					if(data[i].a_id != null) {
						...
						html += "&nbsp;<span id=\"write_date\">" + imgDisplayTime(data[i].regdate) + "</span><br>"
...
function imgDisplayTime(timeValue) {
	var today = new Date();
