                ...
                <table class="table">                    
                    <tr>
                        <td>
                            ...
                            <br>
                            <div>
                                <a href='javascript:void(0)' onClick="return admin_qna_save_comment('${qnaVO.qseq }')" class="btn pull-right btn-success">등록</a>
                            </div>
                        </td>
                    </tr>
                </table>
 ...
 * 상품평 등록
 */
function admin_qna_save_comment(qseq) {
	if(document.commentForm.content.value=='') {
		...
		$.ajax({
			type: 'POST',
			url: 'admin_qna_save_comment',
			data: $("#commentForm").serialize(),
			success: function(data) {
				if(data="success") {	// 댓글 등록 성공
					getAdminQnaCommentList();	// 댓글 목록 요청 함수 호출
					$("#content").val("");
				}
			},		
 ...
 */
$(function() {
	getAdminQnaCommentList();
});

/*
 * 댓글 목록 불러오기
 */
function getAdminQnaCommentList() {
	$.ajax({
		type: 'GET',
		url: 'admin_qna_comment_list',
		dataType: "json",
		data: $("#commentForm").serialize(),
			...
			if(data.length > 0) {
				for(i=0; i<data.length; i++) {
					if(data[i].a_id != null) {
						...
						html += "&nbsp;<span id=\"write_date\">" + adminQnaDisplayTime(data[i].regdate) + "</span><br>"
...
function adminQnaDisplayTime(timeValue) {
	var today = new Date();
